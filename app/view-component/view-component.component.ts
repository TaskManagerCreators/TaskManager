...
export class DialogOverviewExampleDialog {
  ...
  ngOnInit(){
    ...
    /*this.userControl.valueChanges.subscribe(a=>{
      this.http.get('http://localhost:8081/users').subscribe(data => {
        this.users = data;
      });
    });*/

    //this.users = this.userControl.valueChanges.pipe(startWith('') , map(data => this.getUsers(data)));
    this.userControl.valueChanges.subscribe(data=>{
    this.users =  this.http.get('http://localhost:8081/users?name='+data);
  });
  }

  postData(name: string, describe: string, date: string, time: string, type: string, value: string, contacts: string , user: string , notificationInterval: long) {
    let uuid = UUID.UUID();
